---
name: exa
description: "Exa web search and deep research via curl (no MCP). Default: POST /search with highlights. Use for any live web lookup, current events, docs, papers, competitor/company intel, or multi-step research; escalate to type=deep-reasoning or the Agent API for synthesis across many sources. Also: contents extraction from known URLs, answer, monitors, SDKs."
metadata:
  source: https://github.com/exa-labs/agent-skills
---

# Exa

Key: `$EXA_API_KEY` (env). Header `x-api-key: $EXA_API_KEY`. A thin CLI `exa` (`~/.local/bin/exa`) wraps the calls below: `exa search "<q>" [-n N] [-t deep-reasoning] [--text|--summary] [--domains a.com] [--since YYYY-MM-DD] [--json]`, `exa contents <url...>`, `exa research "<task>" --effort low|auto|high [--schema f.json]`, `exa status`. Use the CLI when it fits; drop to curl for anything else.

## Depth ladder — pick the cheapest that answers

| Need | Call |
| --- | --- |
| Quick fact / find pages | `/search` `type:auto` + `contents.highlights:true` (default) |
| Latency-critical | `type:fast` or `instant` |
| Compare / synthesize across sources | `type:deep` (+ `additionalQueries`, `outputSchema`) |
| Hard, ambiguous research question | `type:deep-reasoning` |
| List-building, enrichment, multi-hop, follow-ups, structured JSON over a long task | Agent API `POST /agent/runs` → poll until terminal → see `references/agent.md` |
| Already have URLs | `/contents` → `references/contents.md` |
...
## Router — reference leafs (read only what the task needs)

| Task | Read |
| --- | --- |
| Async multi-step research, list-building, enrichment, `previousRunId` follow-ups, polling/SSE | `references/agent.md` |
| Extract text/highlights/summary/links/subpages from known URLs; freshness; per-URL statuses | `references/contents.md` |
| Grounded answer with citations, no own LLM | `references/answer.md` |
| Company, competitor, funding, leadership, market research playbook | `references/company-research.md` |
| ICP lead lists with scoring and CSV output | `references/lead-generation.md` |
| Choosing search type / latency tradeoffs / `exa-research` model names | `references/models-and-modes.md` |
| Query phrasing, systemPrompt vs outputSchema, freshness patterns | `references/prompting-and-patterns.md` |
| Request-shape mistakes and over-specification fixes | `references/common-mistakes.md` |
| Full `/search` field reference | `references/search.md` |
| Endpoint decision workflow, SDK quick starts, pitfalls (upstream build-with-exa) | `references/build-with-exa.md` |
| `exa-py` / `exa-js` naming and methods | `references/sdks.md` |
| OpenAI-compatible `/chat/completions` `/responses` | `references/openai-compat.md` |
| Minimal raw HTTP across all surfaces | `references/http-requests.md` |
| Scheduled recurring search + webhooks | `references/monitors.md` |
| Migrating legacy Websets to Agent API | `references/migrate-websets-to-agent.md` |
