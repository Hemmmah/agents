...
from summarize_campaign import same_except_budget


class InvocationOracle(unittest.TestCase):
    def test_cap_only_compatibility_rejects_behavior_changes(self):
        current = Path(__file__).with_name('run_trials.py').read_text()
        previous = current.replace('(20, 50, 70)', '(20, 50)')
        self.assertTrue(same_except_budget(previous, current))
        self.assertFalse(same_except_budget(previous, current.replace("'42'", "'43'")))
    ...
    def test_budget_cannot_reset_on_revision(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for n in range(20, 50):
                self.assertEqual(reserve_campaign(path, {'case': 'test', 'subject_sha256': str(n)}, Path(folder)), n+1)
            with self.assertRaises(SystemExit):
                reserve_campaign(path, {'case': 'test', 'subject_sha256': 'another-revision'}, Path(folder))
            self.assertEqual(len(json.loads(path.read_text())['runs']), 50)
