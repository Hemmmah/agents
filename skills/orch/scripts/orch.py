...
def mutation_lock(path: Path):
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.is_symlink():
        raise SystemExit('refusing to mutate a symlink state path')
    lock = path.with_name(path.name + '.lock')
    try:
        descriptor = os.open(lock, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
    except FileExistsError:
        raise SystemExit(f'state locked: {lock}; inspect the owning process before recovery')
    try:
        with os.fdopen(descriptor, 'w') as handle:
...
def validate(state: dict[str, Any]) -> list[str]:
    ...
    if state.get('mode') not in {'plan', 'auto', 'execute', 'monitor'}:
        errors.append('invalid mode')
    if not isinstance(state.get('root'), str) or not Path(state['root']).is_absolute():
        errors.append('root must be an absolute path')
    if not isinstance(state.get('tick'), int) or state.get('tick', -1) < 0:
        errors.append('tick must be a nonnegative integer')
    for field in ('decisions', 'blockers', 'evidence'):
        if not isinstance(state.get(field), list) or any(not isinstance(x, dict) for x in state.get(field, [])):
        ...
        for branch in branches:
            if isinstance(branch, dict):
                ...
                if not isinstance(needs, list) or any(not isinstance(x, str) for x in needs):
                    errors.append(f"branch {branch.get('id', '?')} needs must be a list of IDs")
                    continue
                for dependency in needs:
                    if dependency not in known:
                        errors.append(f"branch {branch.get('id', '?')} needs unknown branch {dependency}")
        if not errors:
            graph = {branch['id']: branch['needs'] for branch in branches}
            pending = set(graph)
            while pending:
                ready = {key for key in pending if not pending.intersection(graph[key])}
                if not ready:
                    errors.append('dependency cycle: ' + ', '.join(sorted(pending)))
                    break
                pending -= ready
...
def ready_ids(state: dict[str, Any]) -> list[str]:
    complete = {b['id'] for b in state['branches'] if b['status'] == 'complete'}
    return [b['id'] for b in state['branches']
            if b['status'] in {'pending', 'running'} and set(b['needs']) <= complete]


def derive_status(state: dict[str, Any]) -> str:
    if all(b['status'] == 'complete' for b in state['branches']):
        return 'complete'
    if ready_ids(state):
        return 'running'
    if any(b['status'] == 'deferred' for b in state['branches']):
        return 'paused'
    return 'blocked'
...
def cmd_plan(args: argparse.Namespace) -> int:
    ...
    if path.exists():
        raise SystemExit('state already exists; resume it instead of replacing the chain')
...
def cmd_tick(args: argparse.Namespace) -> int:
    ...
    if branch is None:
        ...
        return 2
    outcome = 'complete' if args.complete and args.outcome == 'progress' else args.outcome
    if args.complete and args.outcome not in {'progress', 'complete'}:
        raise SystemExit('--complete cannot override a blocker or decision')
    if not args.note or not args.note.strip():
        raise SystemExit('record the observed evidence or unresolved question with --note')
    if args.event_id:
        prior = next((e for e in state['evidence'] if e.get('event_id') == args.event_id), None)
        if prior:
            if (prior['branch'], prior['outcome'], prior['note']) != (args.branch, outcome, args.note):
                raise SystemExit('event ID already records a different result')
            print(json.dumps(state, indent=2, sort_keys=True))
            return 0
    if state['status'] == 'complete' or branch['status'] in {'complete', 'deferred', 'blocked'}:
        raise SystemExit('branch is terminal or held; resolve the recorded hold explicitly')
    if outcome in {'progress', 'no_change', 'complete'} and args.branch not in ready_ids(state):
        raise SystemExit('branch prerequisites are not complete')
    branch["status"] = {
        "progress": "running",
        "no_change": branch['status'],
        "blocked": "blocked",
        "decision_needed": "deferred",
        "complete": "complete",
    }[outcome]
    event = {"at": now(), "branch": args.branch, "outcome": outcome,
             "note": args.note, "event_id": args.event_id}
...
def cmd_resolve(args: argparse.Namespace) -> int:
    ...
    branch = next((b for b in state['branches'] if b['id'] == args.branch), None)
    if not branch or branch['status'] not in {'blocked', 'deferred'} or not args.note.strip():
        raise SystemExit('resolve requires a held branch and the actual decision or changed prerequisite')
    for field in ('decisions', 'blockers'):
        for item in state[field]:
            if item['branch'] == args.branch and 'resolution' not in item:
                item['resolution'] = args.note
    branch['status'] = 'pending'
    state['evidence'].append({'at': now(), 'branch': args.branch, 'outcome': 'resolved', 'note': args.note})
    state['status'] = derive_status(state)
    state['tick'] += 1
    state['updated_at'] = now()
    save(path, state)
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if errors:
        raise SystemExit('; '.join(errors))
    state['ready_branches'] = ready_ids(state)
    print(json.dumps(state, indent=2, sort_keys=True))
    return 0
...
def parser() -> argparse.ArgumentParser:
    ...
    tick.add_argument('--event-id', help='Stable ID for idempotent replay of the same observation')
...
if __name__ == "__main__":
    ...
    if parsed.command in {'plan', 'tick', 'resolve'}:
        with mutation_lock(Path(parsed.state).expanduser()):
